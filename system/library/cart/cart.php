...
class Cart {
	...
	public function removeProduct($product_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE product_id = '". (int)$product_id ."' AND api_id = '" . (isset($this->session->data['api_id']) ? (int)$this->session->data['api_id'] : 0) . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "'");
	}
	...
	public function getProducts() {
		...
		$cart_query =
		$this->db->query("SELECT * FROM " . DB_PREFIX . "cart ".
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				foreach (json_decode($cart['option']) as $product_option_id => $value) {
					...
					if ($option_query->num_rows) {
						...
						elseif ($option_query->row['type'] == 'option_qty') {                        
									...
									$option_value_query = $this->db->query(
									  ...
									  " AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "'");
										...
										if ( ($option_value_query->num_rows) && (!empty($option_value_quantity)) ) {                                                                                                                                               
											...
											$pprice = 0 ; //($option_value_query->row['pspecial'] ? $option_value_query['pspecial'] : $option_value_query['pprice']);
											if($option_value_query->row['pspecial'])  $pprice = $option_value_query->row['pspecial'] ;
											else  $pprice = $option_value_query->row['pprice'] ;
											...
											if ( !isset($option_price_with_quantity) ) 
											  $option_price_with_quantity = 0;
											if ($option_value_query->row['price_prefix'] == '+') {
                                              $option_price_with_quantity += $pprice * $option_value_quantity;
                                              $option_price_quantity += $option_value_quantity;
				 ...
				 if (isset($option_price_with_quantity) && isset($option_query->row['type']) && $option_query->row['type'] == 'option_qty') {
                    $price = 0;
                   // if($option_value_quantity)
                    $newPiecePrice = $option_price_with_quantity / $option_price_quantity;
					$newprice =  $option_price_with_quantity;
					$newtotal = $option_price_with_quantity;

					} else {
                    $price = $product_query->row['price'];
                    $newPiecePrice = ($price + $option_price) / $cart['quantity'];
					$newprice = $price + $option_price;
					$newtotal = ($price + $option_price) * $cart['quantity'];
				...
				//	'total'           => ($price + $option_price) * $cart['quantity'],
					...
					'pprice'          => $product_query->row['price'],
					'recurring'       => $recurring
				);
			} else {
