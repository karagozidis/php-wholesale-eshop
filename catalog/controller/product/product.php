	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($product_info['quantity'] <= 0) {
				...
				//$data['textstock'] = $this->language->get('text_outstock');
			...
			foreach ($results as $result) {
				$data['images'][] = array(
				    ...
				    'imgHex' => $result['imgHex']
				);
			}
			
			$results = $this->model_catalog_product->getDistinctProductImageHexs($this->request->get['product_id']);
			foreach ($results as $result) {
				$data['imageHexs'][] = array(
				    'imgHex' => $result['imgHex']
				);
			}
            
            $data['ImgHexsTotal'] = $this->model_catalog_product->getTotalDistinctProductImageHexs($this->request->get['product_id']);
				...
				foreach ($option['product_option_value'] as $option_value) {
					if (!$option_value['subtract'] || ($option_value['quantity'] > -1)) {
						...
						$product_option_value_data[] = array(
							...
							'rem_qty' => $option_value['rem_qty'], 	
							
							'price_value_tax'               => $price,  //$option_value['price'],
							'price_value'                   =>  $price,  //$option_value['price'],
							...
							'price'                   => $price,  //$this->tax->calculate($price, 1, $this->config->get('config_tax'))  ,
			...
			foreach ($results as $result) {
                ...
                if ($result['location'] == '') 
                {
                    $CCCIMAGE =  'image/manufacturer/access.png';
                } else {
                    $CCCIMAGE =  'image/manufacturer/'. $result['location'];
                }
				...
				$data['products'][] = array(
                    ...
                    'sku'               => $result['sku'],
                    'ImgHexs'           => $result['ImgHexs'],
                    'ImgHexsTotal'      => $result['ImgHexsTotal'],
