<?php
class ControllercatalogfilterOrderBy extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/filterOrderBy');
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_filter->editFilterOrderBy($this->request->post['filter_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('catalog/filterOrderBy', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	protected function getList() {
		...
		$filter_total = $this->model_catalog_filter->getTotalFilterGroupsOrderBy();

		$results = $this->model_catalog_filter->getFilterGroupsOrderBy();

		foreach ($results as $result) {
			$data['filters'][] = array(
				'filter_id' => $result['filter_id'],
				'filterGroupName'            => $result['filterGroupName'],
				'filterName'      => $result['filterName'],
				'edit'            => $this->url->link('catalog/filterOrderBy/edit', 'user_token=' . $this->session->data['user_token'] . '&filter_id=' . $result['filter_id'] . $url, true)
			);
		}
		...
		$this->response->setOutput($this->load->view('catalog/filter_list_OrderBy', $data));
	}
	...
	protected function getForm() {
		...
		//	$data['action'] = $this->url->link('catalog/filter/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
			...
			$data['action'] = $this->url->link('catalog/filterOrderBy/edit', 'user_token=' . $this->session->data['user_token'] . '&filter_group_id=' . $this->request->get['filter_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('catalog/filterOrderBy', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['filter_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$filter_group_info = $this->model_catalog_filter->getFilterGroup($this->request->get['filter_id']);
		}
		...
		if (isset($this->request->post['filter_group_description'])) {
			...
			$data['filter_group_description'] = $this->model_catalog_filter->getFilterGroupDescriptions($this->request->get['filter_id']);
		} else {
		...
		if (isset($this->request->post['filter'])) {
			...
			$data['filters'] = $this->model_catalog_filter->getFilterOrderBy($this->request->get['filter_id']);
		} else {
			$data['filters'] = array();
		}
		...
		$this->response->setOutput($this->load->view('catalog/filterOrderBy_form', $data));
	}
